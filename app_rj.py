...
# ---------------------------------------------------------------------------
# Splash screen — exibida uma única vez por sessão, enquanto os dados
# (parquet/geojson) são carregados e cacheados pela primeira vez. Nas
# próximas execuções (reruns por causa de filtros/widgets) o cache do
# @st.cache_data já responde na hora e a splash não aparece de novo.
# ---------------------------------------------------------------------------
CAMINHO_SPLASH = os.path.join(os.path.dirname(__file__), "assets", "splash_vander22_rio.png")
...
if "splash_exibida" not in st.session_state:
    ...
    with splash.container():
        ...
        if os.path.exists(CAMINHO_SPLASH):
            st.image(CAMINHO_SPLASH, use_container_width=True)
        st.markdown(
            "<p style='text-align:center; opacity:.65; margin-top:-0.5rem;'>"
            "Carregando dados...</p>",
            unsafe_allow_html=True,
        )
